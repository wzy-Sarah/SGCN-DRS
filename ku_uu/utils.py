...
def setup_features(args, drug_id=None):
    ...
    if args.general_features:
        ...
        fp = open("/mnt/wzy/SGCN-master/data/Drug-smiles-mol_MACCS.pkl","rb")
        X = pickle.load(fp)
        X = np.array(X)
        print(X.shape)
...
def generate_drug_mol_feature_():
    id_to_smiles={}
    with open("/mnt/wzy/SGCN-master/data/dds_drug_smiles.csv", 'r') as fp:
            fp.readline()
            for line in fp:
...
def split_KK_KU_UU(order,div_order, i):
    dataset = np.loadtxt("/mnt/wzy/SGCN-master/data/labels.txt")
